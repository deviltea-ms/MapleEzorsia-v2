...
const char* MainMain::m_sRedirectIP = "127.0.0.1";
//paste your custom exp table's array into the place of v83ArrayForCustomEXP
//custom exp table and level numbers MUST MATCH server numbers
...
MainMain::MainMain(std::function<void()> pPostMutexFunc)
{
	...
	std::filesystem::path BfilePath2("Data/zmap.img");
	if (!std::filesystem::exists(BfilePath) && !std::filesystem::exists(BfilePath2)) { 
		Sleep(20);
		SuspendThread(MainMain::mainTHread);
		MessageBox(NULL, L"Either Base.wz is missing from your game directory OR you are loading from .img and zmap.img is not in your Data directory, please reinstall and make sure relevant file(s) exist", L"missing .wz/.img file", 0);
		ExitProcess(0);
	}
	if (MainMain::CustomLoginFrame) {
		MainMain::ownLoginFrame = true; MainMain::bigLoginFrame = true;
	} //use own login if true
	...
	if (std::filesystem::exists(EfilePath)) { 	//only check after "if false" on Client::CustomLoginFrame or things break
		MainMain::EzorsiaV2WzIncluded = true;
		MainMain::CustomLoginFrame = true;
		MainMain::usingEzorsiaV2Wz = true;
	}
	else if(std::filesystem::exists(EfilePath2)){
		MainMain::EzorsiaV2WzIncluded = true;
		MainMain::CustomLoginFrame = true;
	}
	else {
	...
	//Memory::UseVirtuProtect = reader.GetBoolean("general", "UseVirtuProtect", true);//breaks without it so i removed the option, too many options anyway and this wasnt helping 
	const char* serverIP_Address = Client::SERVER_IP_ADDRESS.c_str();
	MainMain::m_sRedirectIP = serverIP_Address;
	unsigned int sleepySleepy = Client::DEBUG_SLEEP_TIME;
	...
	if (strcmp(MainMain::use_custom_dll_1, "CUSTOM.dll")) { //load auxilliary dll files
		...
		if (!hModule) {
			Sleep(20);
			SuspendThread(MainMain::mainTHread);
			MessageBox(NULL, L"Failed to find the first custom dll file", L"Missing file", 0);
			ExitProcess(0); }
	}
	if (strcmp(MainMain::use_custom_dll_2, "CUSTOM2.dll")) {
		...
		if (!hModule1) {
			Sleep(20);
			SuspendThread(MainMain::mainTHread);
			MessageBox(NULL, L"Failed to find the second custom dll file", L"Missing file", 0);
			ExitProcess(0);
		}
	}
	if (strcmp(MainMain::use_custom_dll_3, "CUSTOM3.dll")) {
		...
		if (!hModule2) { 
			Sleep(20);
			SuspendThread(MainMain::mainTHread);
			MessageBox(NULL, L"Failed to find the third custom dll file", L"Missing file", 0);
			ExitProcess(0);
		}
	}
